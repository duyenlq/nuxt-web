<template>
    ...
    <div class="container mt-2 mb-4 md:mt-4 flex flex-col gap-2 md:gap-4">
        ...
        <div class="flex flex-col md:flex-row gap-10"> 
            ...
            <div class="border-2 rounded-xl w-full md:w-3/4 pb-4 pt-1"> 
                ...
                <div class="h-full p-2 px-4"> 
                    <div v-for="items,index in userList" :key="index" :class="{'border-b-2':index == userList.length-1}" class="flex items-center hover:bg-slate-200/50 cursor-pointer duration-300 border-t-2 p-1">
                        ...
                        <div class="w-1/6 flex gap-2">
                            <button @click="editData(items.id, items.username, items.value)" class="text border-slate-200 border-2 p-1 rounded-lg hover:bg-main duration-300">edit</button>
                            <button @click="deleteData(items.id)" class="text border-slate-200 border-2 p-1 rounded-lg hover:bg-main duration-300">delete</button>
                        </div>
                    </div>
                </div>
            </div>
